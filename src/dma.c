...
void MyDMA_Init(uint32_t AddrA, uint32_t AddrB, uint16_t Size)
{
	...
	DMA_InitStructure.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Byte;  // 每次转运的数据宽度，字节传输
	DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Enable;          // 转运完成一次之后地址自增
	
	DMA_InitStructure.DMA_MemoryBaseAddr = AddrB;           // 目的站点基地址，后面两个参数原理与前面一样
...
void MyDMA_Transfer(void)
{
	...
	DMA1_Channel1->CNDTR = MyDMA_Size; //注意根据手册描述，这个是一个递减计数器，指示剩余要传输的数据个数
